...
def load_network(name, opt):
    ...
    opt.repvit_variant = config.get('repvit_variant', 'repvit_m1_1')
    if opt.use_dense:
    ...
    if opt.views == 2:
        model = two_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, repvit_variant=opt.repvit_variant)
    elif opt.views == 3:
        if opt.LPN:
            model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, LPN=True, block=opt.block, repvit_variant=opt.repvit_variant)
        else:
            model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, repvit_variant=opt.repvit_variant)

    if 'use_vgg16' in config:
        opt.use_vgg16 = config['use_vgg16']
        if opt.views == 2:
            model = two_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16 = opt.use_vgg16, repvit_variant=opt.repvit_variant)
            if opt.LPN:
                model = two_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16 = opt.use_vgg16, LPN = True, block=opt.block, repvit_variant=opt.repvit_variant)
        elif opt.views == 3:
            model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16 = opt.use_vgg16, repvit_variant=opt.repvit_variant)
