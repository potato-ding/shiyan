...
parser.add_argument('--LPN', action='store_true', help='use LPN+RepViT' )
...
if opt.views == 2:
    if opt.LPN:
        model = two_view_net(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16=opt.use_vgg16, LPN = True, block=opt.block, repvit_variant=opt.repvit_variant)
    else:
        model = two_view_net(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16=opt.use_vgg16, repvit_variant=opt.repvit_variant)
elif opt.views == 3:
    model = three_view_net(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, VGG16=opt.use_vgg16, repvit_variant=opt.repvit_variant)
