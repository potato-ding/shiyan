...
REPVIT_MODEL_DIR = os.path.join(os.path.dirname(__file__), '..', 'RepViT', 'model')
if REPVIT_MODEL_DIR not in sys.path:
    sys.path.append(REPVIT_MODEL_DIR)
import repvit as repvit_model
...
class RepViTFeatureBackbone(nn.Module):
    def __init__(self, variant='repvit_m1_1'):
        super(RepViTFeatureBackbone, self).__init__()
...
class ft_net_cvusa_LPN(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', block=6, row=False, repvit_variant='repvit_m1_1'):
        super(ft_net_cvusa_LPN, self).__init__()
        # stride is kept for API compatibility; RepViT has fixed stage strides.
        _ = stride

        self.pool = pool
        self.model = RepViTFeatureBackbone(variant=repvit_variant)
        ...
        if init_model!=None:
            self.model = init_model.model
            self.pool = init_model.pool
            self.out_channels = init_model.out_channels
            #self.classifier.add_block = init_model.classifier.add_block
...
class ft_net_cvusa_LPN_R(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', block=6, repvit_variant='repvit_m1_1'):
...
# Define the RepViT-based Model
class ft_net(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', repvit_variant='repvit_m1_1'):
...
# Define the RepViT-based part Model
...
class two_view_net(nn.Module):
    def __init__(self, class_num, droprate, stride = 1, pool = 'avg', share_weight = False, VGG16=False, LPN=False, block=2, repvit_variant='repvit_m1_1'):
        ...
        if VGG16:
            ...
            # RepViT LPN cvusa/cvact
            self.model_1 =  ft_net_cvusa_LPN(class_num, stride=stride, pool = pool, block=block, repvit_variant=repvit_variant)
            if self.sqr:
                self.model_1 = ft_net_cvusa_LPN_R(class_num, stride=stride, pool=pool, block=block, repvit_variant=repvit_variant)
        ...
        if share_weight:
            ...
            if VGG16:
                ...
                self.model_2 =  ft_net_cvusa_LPN(class_num, stride = stride, pool = pool, block=block, row = self.sqr, repvit_variant=repvit_variant)
        repvit_dim = self.model_1.out_channels if not VGG16 else 512
        if not VGG16 and pool == 'avg+max':
