...
if opt.views == 2:
    if opt.LPN:
        model = two_view_net(len(class_names), droprate = opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, LPN = True, repvit_variant=opt.repvit_variant)
    else:
